...
The backend is prepared. You only need to create the project in your own account and run the supplied SQL. There is no server code to write, no signup system to configure, and no table to build manually. For an existing collector, run `generator.sql` to add the generator; a new project can use `schema.sql`.
...
3. Open **SQL Editor** in the project. Create a new query. Copy the entire contents of `supabase/schema.sql` from this repository into it and click **Run**. A successful run creates the private table and the operations the website uses. Do not use the example SQL from a tutorial.
...
- `draw_rejection`: returns one approved public copy or silence result, avoiding an immediate repeat when possible.
- Review and approval happen in Supabase’s Table Editor; see `REVIEW.md`. Originals remain private, and no admin application is needed.
